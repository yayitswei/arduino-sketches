...
void parse_song(char* song) {
  ...
  for (unsigned int i=0; i<1000; i++) {
    ...
    // Read tempo
    int base_duration = analogRead(TEMPO_PIN)/10 + 100; // Minimum will be 100 ms
    ...
    if (command == 'O') {
      ...
      play_rest(base_duration * param);
    } else if ((command >= 'a' && command <= 'g') || (command >= 'A' && command <= 'G')) {
      play_note(command, octave, base_duration * param);
    }
  }
}
